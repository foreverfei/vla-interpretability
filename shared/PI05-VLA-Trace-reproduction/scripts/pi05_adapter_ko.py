...
class Pi05PolicyKO:
    def __init__(self, request):
        ...
        self._ko_hooks = []
        print(f"Pi05PolicyKO loaded on {self.device}, state_norm: MEAN_STD, action_unnorm: MEAN_STD")

    def reset(self): pass
    def configure_knockout(self, config):
        layers = self.model.paligemma_with_expert.gemma_expert.model.layers
        for i, orig in self._ko_hooks: layers[i].self_attn.forward = orig
        self._ko_hooks = []
        if not config or config.get("mode","baseline") == "baseline":
            print("[knockout] baseline"); return
        mode = config.get("mode")
        kl = config.get("knockout_layers",[])
        if isinstance(kl, str):
            kl = [int(x) for x in kl.split(",")] if kl != "all" else list(range(len(layers)))
        kl = [l for l in kl if 0 <= l < len(layers)]
        print(f"[knockout] mode={mode} layers={kl}")
        NEG_INF = -2.3819763e38
        for i in kl:
            ofn = layers[i].self_attn.forward
            def mw(idx, orig):
                def w(*args, **kwargs):
                    if "attention_mask" in kwargs and kwargs["attention_mask"] is not None:
                        m = kwargs["attention_mask"]
                        if mode == "no_image": m[...,:768] = NEG_INF
                        elif mode == "no_text": m[...,768:780] = NEG_INF
                        elif mode == "no_vl": m[...,768:780,:768]=NEG_INF; m[...,:768,768:780]=NEG_INF
                        kwargs["attention_mask"] = m
                    return orig(*args, **kwargs)
                return w
            layers[i].self_attn.forward = mw(i, ofn)
            self._ko_hooks.append((i, ofn))
...
def create_policy(request): return Pi05PolicyKO(request)
